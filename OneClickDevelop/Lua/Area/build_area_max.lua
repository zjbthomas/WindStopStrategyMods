...
for i = 1, build_cnt do
    cost = cost + util.cost_of_build(area.Develop + (i - 1) * buildDevelopPoint)
end

local is_ok = yes_or_no(string.i18_format("本次建设花费 {0} x {1}, 是否继续？", item_tip_link("银两"), cost))
if not is_ok then
    return -1
end
...
change_area_develop(area, MaxAreaDevelop - area.Develop)
